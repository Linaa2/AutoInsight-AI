...
def _render_cancelled() -> None:
    ...
    st.divider()
    _c1, center, _c3 = st.columns([1, 5, 1])
    with center:
...
def main() -> None:
    ...
    if result is None:
        if st.button(
            ...
            _run_progressive_analysis(df, uploaded_file.name, diagram_area)
            return
        st.info(
...
def _run_progressive_analysis(
    ...
    # ---- Stream nodes ----
    for node_name, node_output in stream_analysis(df, file_name=file_name):
